<template>
  <div class="sao-paulo-state-map" ref="chartContainerElement">
    <inline-svg 
      :src="require('../assets/brazil-sao-paulo-location-map.svg')"
      @loaded="svgLoaded"
      @error="svgLoadError($event)"
      fill="black"
      aria-label="Brazil Sao Paulo State Map"
    ></inline-svg>
  </div>
</template>
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const svgLoaded = () => {
      svgPanZoom('.sao-paulo-state-map svg')
      emit('loaded')
